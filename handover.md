...
## 🔗 화면 확인 필요 (8/31)
- [ ] https://myinsurancecalc.com/states/workers-comp/new-jersey.html — h3 소제목 다수 신규 삽입. 기존 state 템플릿에 h3가 거의 없었으므로 스타일 확인 필요.
- [ ] https://myinsurancecalc.com/states/workers-comp/montana.html — 동일.

---

# 🚨 최우선: Google 8월 스팸 업데이트 직격 (콘텐츠 작업보다 이게 중요하다)
...
| 8/17 | **406** |
| **8/18** | **4** |
| 8/19~8/29 | 1~4 (12일 연속) |

**노출 99% 붕괴. 하루 만에.**

**데이터 지연이 아니다.** 8/24 export와 8/31 export의 8/18~8/21 수치가 완전히 동일하다(4/4/1/2). 8/24 세션에서 내가 "지연이지 붕괴가 아니다"라고 판단했는데 **그 판단은 틀렸다.** 일주일 뒤에도 채워지지 않았으므로 확정된 실측치다.

## 원인 특정
**Google August 2026 spam update가 2026년 8월 18일 오전 9시 27분(PT)에 롤아웃 시작, 8월 21일 완료.** 우리 붕괴일과 정확히 일치한다.

- 타겟: **scaled content abuse** — 순위 조작 목적의 대량 생산 콘텐츠. 누가/무엇이 썼는지는 무관하고 "가치가 거의 없는 다수 페이지"가 기준이다.
- 업데이트가 타겟하지 **않은** 것: link spam, site reputation abuse (구글이 명시).
- 성격: SpamBrain 알고리즘 강등이며 **수동 조치가 아니다.** 따라서 GSC에 경고가 뜨지 않고 **재심사 요청 경로도 없다.**
- 회복: 구글은 수정 후에도 **수개월** 걸릴 수 있다고 안내. 재크롤링·재평가가 필요하고 다음 스팸 시스템 갱신에 좌우된다.

## 우리 데이터로 교차검증 (추정이 아니라 측정했다)
states 350페이지의 **본문(nav/aside 제외) 10-gram 중복률**을 카테고리별로 측정:

| 카테고리 | 같은 카테고리 내 본문 중복 중앙값 |
|---|---|
| business-insurance | **62.3%** |
| renters-insurance | **59.4%** |
| health-insurance | 47.5% |
| life-insurance | 45.0% |
| car-insurance | 43.4% |
| workers-comp | 43.1% |
| home-insurance | 41.7% |
| *(대조) 최근 차별화 블로그 클러스터* | *0.4%* |

**350페이지가 서로 본문의 42~62%를 공유한다.** 스팸 정책이 묘사하는 프로필과 정확히 일치한다. 원인 진단은 사실상 확정으로 봐도 된다.

## 🟢 Bing은 무피해 — 그리고 지금 사업의 전부다
| | 노출 | 클릭 |
|---|---|---|
| Bing (전주 → 이번주) | 970 → **1,178** | 38 → **56** |
| Google (3개월 누적) | 31,850 | 14 |
| Google (8/18~8/29 실질) | ~25 | **0** |

- **Bing 클릭 56개 중 47개(84%)가 states 페이지에서 나온다.** workers-comp 20, business-insurance 7, home 5, health 5, car 4, renters 3, life 3.
- GA4: bing 35 + yahoo 19 + duckduckgo 9 = **63세션** vs google organic **6세션**. 활성사용자 193→198로 오히려 소폭 증가 — Bing이 Google 붕괴를 완전히 상쇄했다.
- 사이트는 정상 가동 중이다(Bing이 정상 크롤·노출 중이므로 다운/robots 문제 아님).

## ⚠️ 여기서 반드시 주의할 역설
**states 350페이지는 동시에 (a) Bing 수익의 84%이자 (b) Google 제재의 원인이다.**

따라서 **noindex나 대량 삭제는 절대 하면 안 된다.** 그건 유일하게 작동하는 채널을 스스로 죽이는 것이다. 해법은 제거가 아니라 **차별화**다.

---

## ✅ 이번 세션(8/31) 작업 — 신규 페이지를 만들지 않았다

**판단**: scaled content abuse 제재를 받는 중에 대량 페이지를 더 얹는 것은 역효과다. 확장 전략 자체는 유지하되, 이번 주는 **지금 실제로 돈이 되는 채널의 상위 수익 페이지를 '주별 실체'로 깊게 차별화**하는 데 자원을 썼다. 이게 제재 대응과 Bing 매출 성장을 동시에 만족하는 유일한 방향이다.

**대상 선정**: Bing 수익 상위 workers-comp 2개 주(합계 노출 59 / 클릭 4). 두 주 모두 **제도 자체가 다른 주와 근본적으로 다르므로** 템플릿이 아니라 실체로 차별화가 가능하다는 점이 결정적이었다.

1. **`states/workers-comp/new-jersey.html`** (1,046→1,976단어, Bing 44노출 3클릭)
   - NJ는 **loss cost가 아니라 full manual rate를 CRIB이 산정하고 DOBI 국장이 승인**하는 administered pricing 주다(약 40개 주의 NCCI loss cost 방식과 다름). 400여 민영사 전부가 같은 승인요율에서 출발하고 주 기금은 없다.
   - **따라서 캐리어 쇼핑으로 기본요율이 안 움직인다.** 협상은 schedule rating credit(상한 25%, 비상선포 시 35%)에서 일어난다 — 이게 NJ 고용주의 최대 레버다.
   - 최저보험료 공식($160 expense constant + 290×manual rate, 상한 $1,200), 2026 임원 급여 상하한(주 $3,430 = 연 $178,360 ~ 주 $860), Second Injury Fund 3.58% / UEF 0.00%(잔액 초과로 정지).
   - 미가입 적발이 **CRIB DB 교차대조**로 자동 이뤄지고, 벌칙이 법인이 아니라 **임원 개인 자산에 Superior Court 유치권**으로 간다는 점.
2. **`states/workers-comp/montana.html`** (1,012→2,064단어, Bing 15노출 1클릭 + Google striking distance 28@25.9로 수주째 이월돼 있던 건)
   - MT는 독점주가 **아니다**(ND·OH·WA·WY와 혼동 빈번). MSF는 1990년 설립 **guaranteed market**으로 어떤 고용주도 거절할 수 없고, 민영사는 거절할 수 있다 → 손해이력 깨끗하면 민영 비교, 위험군이면 MSF가 유일 창구. 배당 존재.
   - **ICEC(독립계약자 면제증) 미확인 시 발주자가 부상 책임을 진다** — MT 최대 함정이며 보험이 아니라 검증 실패의 문제.
   - 39-71-401 MCA 면제 대상, 그리고 "신고하지 않으면 기본값이 피보험자"라는 함정.
   - 면제 subcontractor가 결국 ghost policy(연 $1,000~2,000)를 사는 이유 — **면제증으로는 waiver of subrogation을 줄 수 없다.** 8/26 B2B 클러스터와 자연 연결됨.

**효과 측정**: 두 페이지의 타 주 대비 본문 중복률 **48% → NJ 24.1% / MT 22.9%로 반감**. FAQ NJ 4→6, MT 4→7. 전부 출처 확인 가능한 주별 사실이며 생성형 변형이 아니다.

**검증**: 456파일 / ld+json 파싱 실패 0, div·table 밸런스 0, 깨진 내부링크 0, sitemap 455 유효.

## 신규분 색인 현황
8/19 이후 만든 9페이지(위험취미 4 + EV 1 + B2B 4) **전부 Google 노출 0**. 미색인 목록에는 위험취미 3건만 올라 있고 나머지는 목록에도 없다. 스팸 업데이트 이후 신규 페이지가 정상 평가받지 못하는 상태로 보인다. **이 상태에서 신규를 더 만들어도 회수되지 않는다**는 게 이번 판단의 근거이기도 하다.

---

## 🎯 다음 작업 우선순위 (8/31 확정) — 전략이 바뀌는 지점

**P0. 사용자 확인 필요 — GSC "수동 조치"와 "보안 문제" 탭을 직접 볼 것.**
알고리즘 강등이면 경고가 없지만, 만약 **수동 조치가 떠 있다면 대응이 완전히 달라진다**(재심사 요청 경로가 생김). 이건 내가 볼 수 없는 데이터다. 다음 세션에 이 결과를 알려달라.

**P1. states 페이지 차별화를 계속할 것. 신규 대량 생산은 중단.**
이번에 NJ·MT 2개로 중복률 반감을 실증했다. 같은 방식으로 **Bing 클릭 순서대로** 계속 내려가는 게 맞다. 다음 대상(Bing 클릭/노출 기준): maine(25/3), maryland(6/3), tennessee(11/2), arizona(9/2), texas(5/2), north-dakota(11/1), indiana(7/1).
- **주당 2~3개씩, 실제 주별 제도 차이를 조사해서** 쓸 것. 조사 없이 문장만 바꾸면 의미 없다.
- 우선순위는 **제도가 실제로 특이한 주**(독점기금 ND·OH·WA·WY, 독립요율국 주, 주 기금 있는 주)가 차별화 여지가 커서 효율이 높다.

**P2. business-insurance(62%)·renters-insurance(59%) 카테고리가 최악.**
workers-comp(43%)보다 심하다. 다만 renters는 Bing 클릭 3개뿐이라 후순위. business-insurance는 클릭 7개로 가치가 있으니 P1 다음.

**P3. Google 회복은 수개월 단위로 놓고 볼 것.** 알고리즘 강등은 즉시 회복이 없다. **주간 리포트에서 Google 지표로 작업 성패를 판정하지 말 것** — 당분간 판정 기준은 Bing이다.

**P4. 제휴(Hiscox/CJ) 검토 — 오히려 지금이 적기일 수 있다.**
Bing 클릭 56개, 그중 B2B(workers-comp+business) 27개. Google 붕괴와 무관하게 Bing 매출은 성장 중이다. 8/24부터 올려둔 항목인데 이번 주 데이터로 근거가 더 강해졌다.

**❌ 절대 하지 말 것**:
- **states 페이지 noindex / 대량 삭제** — Bing 클릭 84%가 거기서 나온다. 자해다.
- **신규 대량 페이지 생산** — 제재 원인을 키우는 행위. 차별화된 소량 클러스터는 괜찮지만, 지금은 기존 페이지 차별화가 우선순위가 더 높다.
- Google 지표로 단기 성패 판정.
- workers comp 클래스코드 격자, 신체부위별 보상액(법률 리스크) — 기각 유지.
- GL 계산기 신규, Ezoic, IndexNow 재론(8/19 반영 완료).

## 💰 수익화 현황 (8/31)
- **Google 실질 클릭 0 / Bing 클릭 56.** 사업이 Bing 단일 채널이 됐다는 사실을 직시할 것.
- 다행히 Bing은 성장 중이고(38→56, +47%), 우리 롱테일 state 전략이 Bing에서는 계속 작동한다(가중평균 순위 5.4).
- 1년 데드라인 관점: Google 회복을 기다리는 것보다 **Bing에서 클릭을 늘리고 제휴로 전환하는 게 현실적인 경로**다. 다음 세션부터 판단 기준을 그렇게 잡는 게 맞다.

# MyInsuranceCalc.com 인수인계 (2026-08-26 업데이트, 14회차 — 신규 클러스터)

## 🔗 화면 확인 필요 (8/26)
- [ ] https://myinsurancecalc.com/blog/insurance-requirements-in-contracts.html — **4열 가격표 + blockquote 인용블록** 신규. blockquote는 이 사이트에서 처음 쓴 요소라 스타일이 없을 수 있다. 이번 세션 최대 리스크 지점.
- [ ] https://myinsurancecalc.com/blog/additional-insured-vs-certificate-holder.html — 3열 비교표(빈 헤더 셀 있음).
- 나머지 2건(certificate-of-insurance-explained / hired-non-owned-auto-insurance)은 단순 표라 위 2건만 보면 된다.

## ✅ 이번 세션(8/26): 신규 B2B 클러스터 — 계약서 보험요구조항

GSC를 보지 않고 외부에서 새 클러스터를 찾는 세션이었다(사용자 지시). 후보 4개를 실제로 경쟁강도 조사한 뒤 채택.

| 후보 | 조사 결과 | 판정 |
|---|---|---|
| **계약서 보험요구조항(COI/추가피보험자/HNOA)** | 지배자가 B2B 컴플라이언스 SW와 브로커뿐 | ✅ **채택** |
| 상업임대차 보험요구사항 | Hiscox·Insureon·Vouch + 브로커 다수(Kinro만 3페이지) | ❌ 기각 |
| workers comp 클래스코드 요율 | 주×코드 전용 사이트 격자 커버 | ❌ 기각(8/24 판정 유지) |
| 신체부위별 보상액(PPD) | 로펌 지배 + 법률자문 인접 리스크 | ❌ 기각(8/24 판정 유지) |

**채택 근거 3가지**:
1. **수익성 정조준.** Bing 클릭 38개 중 workers-comp 12 + business-insurance 6 = **18개(47%)가 B2B**다. 이 클러스터의 독자가 정확히 그 사람들이고, 제휴 단가가 가장 높은 트래픽이다.
2. **경쟁의 방향이 반대다.** 검색 상위가 CertFocus·TrackMyVendor·coisoftware·docutrax 등 **"증서를 받는 GC에게 컴플라이언스 SW를 파는 사이트"**와 브로커 리드젠이다. **"요구받는 소상공인 쪽" 관점이 통째로 비어 있다.** lawinsider는 조항 DB라 소비자에게 무용.
3. **우리만의 장치 = 가격을 매긴다.** 경쟁사는 전부 용어를 *설명*만 하고, 누구도 "이 조항이 연간 얼마"를 말하지 않는다. 8/10 Maine/Vermont 작업에서 확보한 GL 실비용 + 이번 조사의 특약 단가를 결합해 전 페이지에 금액을 넣었다.

### 신규 4건
1. **`blog/insurance-requirements-in-contracts.html` (허브, 2,005단어)**
   실제 조항 원문을 인용하고 줄단위 가격표를 붙였다(추가피보험자 $100~500/yr, 대위권포기 <$300, HNOA $150~600, 엄브렐러 $400~1,000).
   - **핵심 통찰: 비싼 건 특약이 아니라 한도(limits)다.** 조항이 비싸 보이는 건 빽빽해서지 비싸서가 아니고, 한도가 유일하게 실질 협상 가능한 항목이다. 협상 논리는 "비례성"(소액 계약에 과대 한도 요구 시 증분보험료를 계약금액에 반영 요청).
   - **30일 해지통보 조항은 ACORD 현행 서식상 보험사가 이행 불가**라 그대로 서명하면 안 된다는 점 — 이건 경쟁사가 거의 안 다루는데 실무에서 가장 흔한 함정.
   - 면책(indemnification)조항이 보험 담보범위를 초과하는 위험 + 주별 anti-indemnity 법령.
2. **`blog/certificate-of-insurance-explained.html` (1,678단어)**
   ACORD 25가 스스로를 "information only"라 명시한다는 점(NY 보험법 근거), 90초 판독 필드표, **총한도(aggregate)는 이미 소진됐을 수 있으나 증서에는 안 나온다**는 점.
3. **`blog/additional-insured-vs-certificate-holder.html` (1,514단어)**
   CG 20 10(진행중 작업) vs CG 20 37(완성 작업) 구분 — **하자소송은 준공 수년 뒤에 터지므로 여기가 실무 최대 함정.** 그리고 **숨은 비용: 추가피보험자는 내 한도를 나눠 쓴다**(경쟁사 전부 미언급). blanket vs scheduled.
4. **`blog/hired-non-owned-auto-insurance.html` (1,531단어)**
   GL은 자동차 면책 / 개인자동차보험은 업무 면책 / 상업자동차는 소유차량만 → **3중 사각지대**. 책임한정(직원 차·직원 부상 미보상)이 최대 오해. respondeat superior. 최근 계약서에서 요구 항목으로 등장 증가 중이라 클러스터와 자연 연결.

### 검증
- 중복검사(shingle 8-gram): 기존 페이지 대비 **최대 0.67%**, 신규 4개 상호 최대 5.59%(사이드바·면책 보일러플레이트).
- 인바운드: 허브 9 / COI 7 / HNOA 6 / 추가피보험자 5 — **전부 5개 기준 충족**.
- 연결 경로: blog/index 카드 4장, freelancer·gig-workers·umbrella·tools/business-insurance 사이드바, **emr-calculator 본문**(EMR 1.00 통과 후 다음 관문이 보험조항), **workers-comp-audit-guide 본문**(하도급자 증서 미비 = 감사 최대 청구원인). sitemap 451→455, llms.txt 4건.
- 456파일 / ld+json 파싱 실패 0, div·table 밸런스 0, 깨진 내부링크 0, FAQPage 22개 전부 본문 h3 1:1 대응.

## 🎯 다음 작업 우선순위 (8/26 확정)
**P0. `how-to-lower-car-insurance.html` Bing 클릭이 0을 벗어났는지 확인.** 8/24 CTR 작업(title/meta 금액 명시 + 누적절감표)의 검증 지점. 사이트 최대 노출 자산(Google 1,176@87 + Bing 726@7)이 양쪽 다 클릭 0이었다. 벗어났으면 → 다른 Bing 상위권 페이지에 같은 처방 확대.
**P1. 이번 B2B 4건 색인 확인.** 8/19 위험취미 4건이 참고치 — scuba는 5일 만에 색인, 나머지 3건은 대기했다. 인바운드 5개 규칙이 계속 유효한지 보는 지점이기도 하다.
**P2. 8/24 EV 신규 1건 + 위험취미 3건 색인·순위 확인.**
**P3. 암 클러스터 분화**(방광암 38@23.6 / 신장암 9@20.2 / 기저세포암 7@27.1) — **4주 연속 이월 중.** 계속 밀리고 있으니 다음엔 실제로 하든가, 아니면 후보에서 빼든가 결정할 것.
**P4. 제휴(Hiscox/CJ) 가입 검토** — 8/24에 올린 항목. 이번 B2B 클러스터가 자리잡으면 랜딩 대상이 생기므로 타이밍이 맞아간다.

**🔶 차순위 신규 클러스터 후보(조사만 하고 보류)**:
- **상업임대차 보험요구사항** — 경쟁 포화라 기각했으나, "임대인이 요구하는 조항 읽는 법"으로 각도를 좁히면 이번 계약서 클러스터의 스포크로는 가능. 단독 페이지로는 하지 말 것.
- 비시민권자·비자소지자 생명보험(8/19 보류), 이혼 판결 생명보험(8/19 보류, 법률 민감).

**❌ 하지 말 것**:
- workers comp 클래스코드 격자, 신체부위별 보상액(법률 리스크) — **2주 연속 기각. 재검토 금지.**
- 미색인 잔여 8건 추가 대응.
- state FAQ 잔여분, states/life-insurance 보강.
- GL 계산기 신규(MoneyGeek 50개주 보유), Ezoic 재검토.
- **IndexNow 재론** — 8/19 반영 완료(`d3b860d`). 색인 주채널은 사이트맵 재제출.

## 💰 수익화 메모
이번 클러스터는 블로그지만 **tools 우선 원칙과 충돌하지 않는다.** 수익 경로가 블로그 자체가 아니라 `tools/business-insurance.html` · `tools/workers-comp.html` · `tools/emr-calculator.html` · `tools/umbrella-insurance.html`로 트래픽을 넘기는 것이고, 4페이지 전부 계산기로 링크를 걸었다. 계약서 조항을 읽는 사람은 **지금 당장 보험을 사야 하는 사람**이라 인텐트가 사이트에서 가장 높다.

# MyInsuranceCalc.com 인수인계 (2026-08-24 업데이트, 13회차 — 주간 정기작업)

## 🔗 화면 확인 필요 (8/24)
- [ ] https://myinsurancecalc.com/blog/ev-battery-total-loss.html — 신규. 표 2개(3열 배터리비용 + 2열 주별기준 비교표). 모바일 확인.
- [ ] https://myinsurancecalc.com/blog/how-to-lower-car-insurance.html — 4열 누적절감표 신규 삽입. 모바일 가로스크롤 확인.

## 🟢 8/17 결론 정정: 내부링크 가설은 옳았다 (기각을 철회한다)

**미색인 21건 → 11건.** 8/17 handover에 "내부링크로는 안 풀린다, 크롤예산 문제"라고 단정했는데 **그 판단이 틀렸다.**

Coverage 차트 일자별 추이가 명확하다:
```
8/14: 21  →  8/15: 8  →  8/17: 8  →  8/18: 11 (신규 위험취미 3건 유입)
```
8/15에 **13건이 한 배치로 색인**됐다. 8/10 내부링크 작업(states 350페이지 상호링크 + long-term-care 고아 해소)의 효과가 약 5일 뒤에 나타난 것이고, 8/17 export는 GSC coverage 데이터 지연(약 3일) 때문에 8/14까지만 반영돼 21로 보였던 것이다.

**교훈 2개, 다음 세션에서 반드시 지킬 것:**
1. **GSC coverage 데이터는 약 3일 지연된다.** export 당일 수치로 2주짜리 작업의 성패를 단정하지 말 것. 8/17에 성급하게 가설을 기각했다가 이번에 되돌렸다.
2. **내부링크 인바운드 확보는 실제로 작동한다.** "신규 페이지 인바운드 5개 이상" 규칙을 계속 유지할 것. 8/19 위험취미 4건 중 **scuba-divers는 5일 만에 색인 완료**(미색인 목록에 없음), 나머지 3건은 대기 중 — 신규 페이지치고 정상 속도다.

**현재 미색인 11건**: 위험취미 신규 3(hobbies/skydivers/extreme-sports) + private-pilots + business-insurance/florida + car-insurance/alaska + health-insurance/virginia + home-insurance/utah + workers-comp/mississippi + workers-comp/new-hampshire + tools/long-term-care-insurance.
→ 신규 3건은 시간 문제. 나머지 8건은 8/10 작업 후에도 안 풀린 잔여분이니 **이제는 진짜로 놔둘 것.**

## 🔴 이번 주 최대 발견: 같은 페이지, Google 87위 vs Bing 7위

`blog/how-to-lower-car-insurance.html` 한 페이지의 성적이다.
...
**사이트 단일 최대 노출 페이지**(합계 약 1,900)인데 양쪽 다 클릭 0이다. 그리고 80위 차이는 콘텐츠 품질로 설명이 안 된다 — 같은 파일이다. **도메인 권위 격차의 가장 깨끗한 증거**이고, 지금까지 추정으로만 말하던 걸 처음으로 단일 변수 대조로 확인한 셈이다.

**실행 판단**: Bing은 이미 클릭 가능 구간(7위)이므로 순위작업이 아니라 **CTR 작업**이 정답이다. title/meta를 구체적 금액으로 교체하고, 경쟁사가 답하지 않는 "다 하면 얼마"를 표로 넣었다. 다음 export에서 이 페이지 Bing 클릭이 0을 벗어나는지가 검증 지점.

## ✅ 나머지 데이터 요약

### Google
3개월 노출 33,975 → **33,367**(-1.8%), 클릭 16 → **15**. 사실상 횡보.
- ⚠️ **8/18~8/21 노출이 4/4/1/2로 붕괴한 것은 데이터 지연이지 트래픽 붕괴가 아니다.** 마지막 완전 주는 8/10~8/16의 **3,114노출**(직전 2,301 → **+35%, 7월 중순 이후 최고**). 다음 세션에서 이 주를 기준선으로 볼 것.
- tools CTR 0.103% vs blog 0.024% — **4배 격차 4주 연속**. 클릭 15개 중 tools 9개. 확정된 사실로 취급해도 된다.
- tools 노출 8,113 → 8,774(+8%), blog 14,153 → 12,612(-11%). tools 우위 계속 벌어지는 중.
- `tools/valuables-insurance.html` **순위 5.67**(48노출 1클릭) — 사이트 최고 순위. 볼륨이 작아 액션은 없지만 기록해둔다.

### Bing (폭증)
노출 44 → **970**(22배), 클릭 12 → **38**, 가중평균순위 **6.66**.
- how-to-lower-car-insurance(726) 제외하면 노출 244 / 클릭 38 / 평균순위 5.43 — **CTR 15.6%**.
- 카테고리별 클릭: **workers-comp 12** / business-insurance 6 / tools 4 / health 4 / car 4.
- **workers-comp가 Bing 최대 수익 채널이다.** Google에서도 클릭 2개. B2B라 제휴 단가도 유리하다.

### GA4 (7/27~8/23)
활성사용자 151 → **193**(+28%). 이벤트 919, 참여시간 34.5초(39.3→34.5 악화). 홈 이탈률 **80.8%**(76.9→80.8, 다시 악화).
- **bing 27 + yahoo 18 + duckduckgo 11 = 56** vs **google 7 + search.google.com 12 = 19**. Bing 계열이 Google의 **약 3배**(전주 2배 → 확대).
- AI 어시스턴트 12세션(copilot 11, chatgpt 1, perplexity 1). llms.txt 유지.

### 8/17·8/19 작업분 경과
- `tools/term-vs-whole.html`: 노출 28 → **81**(+189%), 단 순위 20.4 → 41.7로 악화. Bing에선 3노출 2클릭@7위. **노출은 늘고 순위는 내려간 건 새 쿼리로 확장됐다는 뜻**이라 나쁘게만 볼 필요 없다.
- `tools/emr-calculator.html`: 19노출@42.3 **클릭 1**(Google) + Bing 7노출 1클릭@6.4. 양쪽에서 클릭 발생.
- 위험취미 4건: scuba 색인 완료, 나머지 3건 대기. 아직 노출 0 — 정상.

## 🎯 이번 세션 실행 (커밋 `d182b36`)
1. **`how-to-lower-car-insurance.html` CTR 구조 개선** (2,372→3,035단어) — 위 참조. title/meta 금액 명시 + 누적절감 계산표(\$2,400→\$1,430, 40% 절감, 1단계가 절반 차지) + 주별 격차 섹션(50개주 페이지 연결). FAQ 5→7.
2. **`blog/ev-battery-total-loss.html` 신규** (2,051단어)
   - **선정 근거**: Bing에 EV 배터리 전손 쿼리가 **독립적으로 2건** 등장(둘 다 5위), 그리고 `tools/ev-insurance.html`이 사이트 최고 CTR(58노출 2클릭 **3.45%**). 검증된 니치의 확장이지 신규 도박이 아니다.
   - **경쟁 빈틈**: 전손기준 일반론(Policygenius·WalletHub·Insurify·carinsurance.com)은 EV를 안 다루고, EV 배터리 기사(Liberty Mutual·KBB·UK 매체·손해사정사)는 주별 기준을 안 다룬다. **교차점이 비어 있다.** 우리는 50개 car-insurance 주 페이지가 있어 이 교차점을 구조적으로 소유할 수 있다.
   - 내용: 팩=차량가치 30~40%, 봉인팩 진단데이터 부재로 안전인증 불가→전손이 기본값, 교체비(하이브리드 \$3~6K / 주류EV 설치 \$12~20K), **주별 전손기준 60~100%에서 동일 손상이 정반대 결과를 내는 비교표**, TLF 주 별도, EV 급감가+낮은 전손문턱→gap보험 논거, ACV 협상 6단계, 보증≠보험.
   - 중복검사 최대 0.49%. 인바운드 5개 확보.
3. 검증: 452파일 / ld+json 파싱 실패 0, div·table 밸런스 0, 깨진 내부링크 0, sitemap 451 유효.

## 📊 키워드 리서치 (다음 세션 재활용)
**striking distance(8~32위) = 28쿼리 / 901노출.** 3주 연속 900~1,000선 횡보.

| 클러스터 | 노출 | 순위 | 상태 |
|---|---|---|---|
| DUI | 161 | 26.0 | 8/10 확장, 무반응 |
| Maine 소상공인 | 237 | 25.4 | 8/10 확장, 무반응 |
| 파일럿 | 93 | 31.4 | 대기 |
| 간호사 | 84 | 27.3 | 8/10 확장, 개선 |
| **암(방광/신장/기저세포)** | **58** | **20~27** | **미착수 — 3주 연속 이월** |
| EMR/SC | 45 | 14.2 | ✅ 8/17 고도화 |
| term vs whole | 22 | 22.6 | ✅ 8/17 고도화 |
| Montana state fund | 28 | 25.9 | 미착수 |

**❌ 이번에 조사 후 기각한 신규 클러스터 2개 (재검토 금지)**:
- **workers comp 클래스코드 요율** — Bing 쿼리("clerical worker rate tennessee" 등)가 유도했으나, workerscompensationshop / workerscompcost.com / wcclasscode.com / brightcoastins 등 **주×클래스코드 전용 사이트가 이미 격자로 커버**. 최포화.
- **workers comp 신체부위별 보상액(scheduled injury/PPD)** — Bing "body part values in workers comp delaware"(2위)가 유도했으나 **로펌 사이트가 지배**(Roden·Cruz·Van Camp·Deputy&Mizell·Marker) + compcharts.org·workcompcalc.com 전용사이트. 게다가 **법률자문 인접 영역이라 우리가 손대면 리스크**. 명확히 기각.

**🔍 다음 세션 후보**:
- **암 클러스터 분화** (58노출@20~27) — 3주째 이월 중. 20위권 진입 시 CTR 급등 패턴(colon-cancer 21.3위에서 4.55%)을 감안하면 여전히 최우선.
- **EV 클러스터 2차 확장** — 이번 신규분 성과 확인 후. 후보: 충전기/홈차저 보험, 하이브리드 전용.
- **workers-comp 주 페이지 보강** — Bing 최대 수익 채널(클릭 12). 신규가 아니라 기존 50개 강화 방향. Bing 쿼리가 요구하는 건 "employer 관점 요율"(maryland/tennessee/nj) — 클래스코드 격자는 기각했지만 **주 페이지 안에서 요율 맥락을 두껍게 하는 건 별개**.

## 💰 수익화 판단 (8/24)
- **tools 4배 우위 4주 연속 = 확정.** 신규 자원 배분은 계산기 우선 원칙 유지.
- **Bing이 실질 수익 채널이다.** 클릭 38 vs Google 15. 그리고 Bing workers-comp 클릭 12는 B2B라 제휴 단가가 가장 높은 트래픽이다. **다음 세션 제휴 검토는 workers-comp/business-insurance 쪽부터 볼 것**(Hiscox/Next는 소상공인 대상).
- **제휴 가입 타이밍 재평가 필요**: 8/17까지는 "클릭 절대량 부족으로 시기상조"였는데, Bing 38클릭이 붙으면서 월간 실클릭이 50건대로 올라왔다. **다음 세션에 Hiscox/CJ 가입을 실제로 검토할 것.** 단 여전히 비US 트래픽 비중을 확인하고 판단.
- 1년 데드라인: Google 클릭은 15로 정체지만 **Bing 포함 실질 클릭은 53**. 8/10에 세운 "30 클릭" 기준은 이미 넘겼다. 기준을 Google 단독이 아니라 **전체 검색 클릭**으로 바꿔서 볼 것.

## 🎯 다음 작업 우선순위 (8/24 확정)
**P0. `how-to-lower-car-insurance.html` Bing 클릭이 0을 벗어났는지 확인.** 이번 CTR 작업의 검증 지점. 벗어났으면 → 다른 Bing 상위권 페이지에도 같은 CTR 처방 적용. 안 벗어났으면 → Bing 상위 노출이 클릭으로 안 바뀌는 구조적 이유를 따로 찾아야 한다.
**P1. 위험취미 3건 + EV 신규 1건 색인 확인.**
**P2. 암 클러스터 분화** — 3주째 이월. 이번엔 실제로 할 것.
**P3. 제휴(Hiscox/CJ) 가입 검토** — Bing workers-comp 클릭 기반. 위 수익화 항목 참조.

**❌ 하지 말 것**:
- **미색인 잔여 8건 추가 대응** — 8/10 작업으로 13건 풀렸고 남은 8건은 그 후에도 안 풀린 잔여분. 이제 놔둘 것.
- workers comp 클래스코드 격자, 신체부위별 보상액(법률 리스크).
- state FAQ 잔여분, states/life-insurance 보강(노출 71, 페이지당 3).
- GL 계산기 신규(MoneyGeek 50개주 보유), Ezoic 재검토.
- **IndexNow 재론** — 8/19 반영 완료(`d3b860d`). 색인 주채널은 사이트맵 재제출.
