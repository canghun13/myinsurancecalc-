# MyInsuranceCalc.com 인수인계 (2026-07-11 업데이트)
...
- **오래된 글(FAQ 없음) 발견 시 우선순위 대상**: 아래 12개 블로그 글이 아직 FAQ 없음 (2026-07-11 기준 스캔 결과) — GSC 데이터에서 해당 글들이 노출/볼륨 있으면 우선 보강 후보로 고려:
  - `disability-insurance-guide.html`, `does-car-insurance-cover-ev-battery.html`, `ev-insurance-cost.html`, `freelancer-insurance-guide.html`, `how-much-life-insurance-do-i-need.html`, `how-to-lower-car-insurance.html`, `insurance-for-college-students.html`, `life-insurance-for-remote-workers.html`, `pet-insurance-worth-it.html`, `renters-insurance-for-college-students.html`, `term-vs-whole-life-insurance.html`, `umbrella-insurance-explained.html`
...
## 다음 확인 시점
- **다음 Search Console 색인 커버리지 확인 시**: 350+ 신규 페이지 색인 진행 상황 체크. nevada/new-york 2개가 여전히 미색인이면 그때 실제 보강 착수(현재는 대기).
- 색인이 충분히 따라잡히면 신규 콘텐츠 재개 판단 (조건별 시리즈 3편 고혈압부터, 또는 니치 툴 후보부터)
- GSC 데이터(Performance/Coverage) 받을 때마다 정리→우선순위 매겨서 보강 진행하는 루틴 계속

## 이번 세션 전달 자료 참고
- GSC Performance export(3개월 검색어/페이지 데이터), Coverage export(색인 상태) 분석 완료. 원본 CSV는 재사용 안 해도 됨(이미 분석 반영 완료), 다음번엔 새로 받은 데이터로 분석.
