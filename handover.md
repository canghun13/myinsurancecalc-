...
**1. 애드센스 재검토 요청 타이밍은 Claude(작업 세션)가 판단한다.** 사용자가 명시적으로 위임함 — 매번 물어보지 말고, GSC 클릭/노출 지표가 개선되는 추이를 보고 재검토를 넣을지 스스로 판단할 것. 지난 세션들에서 진행한 기술적 조치: ads.txt 생성, privacy.html 보강, state 350페이지 "How to Save" 팁 실제 차별화, tools 20개 FAQPage 신규 삽입, tools/term-vs-whole.html 레이아웃 버그 수정 — 이 정도면 "가치가 별로 없는 콘텐츠" 반려 사유는 상당 부분 해소됐다고 판단되나, **재검토 요청 자체는 아직 넣지 않은 상태**. 신생 사이트 특성상 트래픽/참여 신호(현재 3개월 누적 클릭 11회 수준)가 낮은 것도 반려에 영향줬을 수 있어, 콘텐츠 수정만으로 100% 통과 보장은 안 됨 — 다음 GSC export에서 클릭이 유의미하게 늘어나는 걸 확인한 뒤 재검토를 넣는 게 안전하다고 판단.
...
**사용자 지시(7/24)**: (1) 애드센스 재검토는 "넣을 때다" 싶으면 Claude가 먼저 알아서 이야기할 것, 매번 물어볼 필요 없음. (2) 애드센스/제휴 마케팅을 전략적으로 구분하지 말 것 — 돈 되는 방법이면 뭐든 병행 진행. (3) 요청받지 않은 분석/작업을 세션 중간에 스스로 벌이지 말 것 — 다음 정기 세션(주간 작업) 때 필요한 것만 처리, 토큰 낭비 금지.
