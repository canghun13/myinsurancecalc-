...
## 🔥 다음 세션 최우선 확인 사항: 공격적 확장 모드 전환 (7/15 사용자 지시)
사용자가 "이제 공격적으로 해야 될 타이밍" 판단 — 신규/보강 둘 다 힘을 실어서 진행하라는 명시적 지시. 이전 세션들의 "모라토리엄", "소규모만", "신중하게" 톤은 더 이상 기본값이 아님. 다음 세션 시작하면:
- 신규 콘텐츠: 조건별 시리즈(수면무호흡증→우울증/불안→비만), 니치 툴(콘도/모바일홈/결혼식/귀중품/최종비용/사이버/중대질병보험) 등 대기 중이던 후보들을 **적극적으로, 여러 개씩** 진행. 매번 "이거 해도 될까요" 되묻지 말고 중복확인+경쟁강도 확인만 거쳐서 바로 실행.
- 보강: FAQ 없는 페이지, 얕은 콘텐츠, workers-comp 51개처럼 유사도 높은 카테고리 등 발견되는 대로 폭넓게 처리. 이전처럼 "노출 상위 3개만" 식으로 좁게 자르지 말 것.
- 단, 품질 기준(실제 조사 데이터, 800단어+, div/JSON-LD 검증)은 그대로 유지 — "공격적"은 속도/범위 얘기지 품질 타협이 아님.
