...
## ⚠️ 현재 최우선 방침: 신규 콘텐츠 전면 중단 (7/15 기준도 유지)
...
- **7/15 재확인**: 색인 245→396(93%)까지 빠르게 진행 중이라 방향은 긍정적이나, "발견됨-미색인" 22건이 아직 크롤 대기 상태라 완전히 소화됐다고 보기엔 이름 → **모라토리엄 한 사이클 더 유지**. 이번 세션 GSC 쿼리 데이터 전수 스캔(노출 상위 260개+ 확인) 결과 새로운 키워드 클러스터/토픽 갭도 발견 안 됨 — 노출 상위 쿼리는 전부 기존 페이지(암시리즈/조종사/DUI/college students/주별 workers-comp·renters·business insurance)가 이미 타겟팅 중. **신규 작성 후보 자체가 없는 상태.**
- **적용 범위**: 신규 블로그, 신규 툴, 신규 state 카테고리/페이지 **전부 중단**.
- **예외**: 기존 페이지 보강(FAQ 추가, 문구 정밀매칭, 버그 수정)은 계속 진행 — 이건 "신규"가 아니라 "기존 자산 개선"이라 방침에 안 걸림.
- **재개 시점**: 다음 Coverage export에서 "발견됨-미색인" 건수가 한 자릿수로 줄어드는 등 신규 350+페이지 색인이 대부분 소화됐다고 판단될 때 재검토. 먼저 나서서 신규 제안하지 말 것 — 물어보면 후보 리스트 정리는 해주되, 실행은 사용자 확인 후.
...
## 다음 확인 시점
- **다음 Coverage export 확인 시**: "발견됨-미색인" 22건이 얼마나 줄었는지가 모라토리엄 해제의 핵심 신호. 한 자릿수로 줄면 신규 콘텐츠 재개 검토. nevada/new-york이 여전히 미색인이면(개별 URL 리스트 확보 시 확인) 그때 실제 보강 착수.
- 색인이 충분히 따라잡히면 신규 콘텐츠 재개 판단 (조건별 시리즈 3편 고혈압부터, 또는 니치 툴 후보부터) — 단, 7/15 기준 GSC 데이터 전수 스캔으로도 새 키워드 갭은 안 보였으므로 재개하더라도 기존 후보 리스트(아래 "신규 콘텐츠 후보 리스트") 순서대로 진행하면 됨
...
- **핵심 발견 1 (색인)**: 색인 245→396(93%)로 급증, 신규 350+페이지가 실제로 빠르게 크롤링·색인되고 있음 확인. 다만 "발견됨-미색인" 22건이 크롤 대기 중이라 모라토리엄은 한 사이클 더 유지.
...
- **핵심 발견 3 (키워드 갭 스캔)**: 검색어 260개+ 전수 확인, 새로운 토픽 클러스터 없음. michigan/kentucky 등 주별 state 쿼리도 기존 페이지 문구와 이미 잘 매칭됨. → 신규 콘텐츠 후보 없음, 이번 세션 신규 작성 안 함.
- **이번 세션 실행 작업**: 노출 상위이면서 FAQ 없는 페이지 3개(`tools/health-insurance-self-employed.html`, `tools/term-vs-whole.html`, `states/business-insurance/maine.html`)에 FAQ 4~5개 + FAQPage 스키마 추가. 상세는 위 "최근 대규모 작업 이력" 9번 참고.
