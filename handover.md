...
## 🔗 화면 확인 필요 (9/8)
- [ ] https://myinsurancecalc.com/states/workers-comp/texas.html — **FAQ 섹션이 이 페이지에 처음 생겼다**(기존엔 없었음). 위치가 Related Tools 앞으로 들어갔는지, h3 스타일이 다른 주 페이지와 같은지 확인.
- [ ] https://myinsurancecalc.com/states/workers-comp/north-dakota.html — h3 소제목 다수 신규. 스타일 확인.

---

## 🔴 이번 주 핵심: 신규 페이지는 지금 회수가 0이다 (데이터로 확정)

8/19 이후 3주간 발행한 **12페이지**(위험취미 4 + EV 1 + B2B 4 + 무직원계약자 3)의 성적:

| | 결과 |
|---|---|
| **Google** | 12개 중 **11개가 '발견됨-현재 색인 생성되지 않음'**. 최종 크롤링 날짜가 전부 `1970-01-01` = **한 번도 크롤된 적 없음** |
| **Bing** | 12개 중 노출이 잡힌 건 `ev-battery-total-loss` **1개뿐**(5노출 0클릭) |

반면 **8/31에 '보강'한 기존 2페이지**는 즉시 반응했다:

| 페이지 | 8/31 이전 | 9/8 | 변화 |
|---|---|---|---|
| states/workers-comp/new-jersey | Bing 44노출 3클릭 @6.27 | **75노출 4클릭 @5.80** | **노출 +70%, 순위 개선** |
| states/workers-comp/montana | 15노출 1클릭 | 15노출 1클릭 @4.20 | 순위 개선 |

**결론: 신규 발행 12개 = 사실상 0. 기존 심화 2개 = +70%.**

미색인 건수도 **11 → 19로 증가**했고, 증가분이 전부 신규 발행분이다. 즉 Google은 이 도메인의 신규 URL을 발견은 하되 크롤 순위에서 계속 밀어내고 있다.

**→ 이번 주 자원을 전부 심화에 배분한 근거다.** 확장 전략 자체는 유지하되, **확장의 축을 '개수'에서 '깊이'로 옮긴다.** 다음 세션도 이 기조를 유지할 것. 신규를 만들지 말라는 뜻이 아니라, 같은 시간을 쓸 때 회수가 나오는 쪽이 명확해졌다는 뜻이다.

## 📉 Google 현황 (회복 없음, 계속 악화)
주간 노출: 3,114(8/10~16) → 421 → 18 → **9**(8/31~9/6). 3개월 클릭 12.
8/18 스팸 업데이트 피격 이후 3주째 바닥이고 **반등 신호 없다.** 8/31에 적은 대로 회복은 수개월 단위로 놓고 볼 것.
...
- how-to-lower-car-insurance(734노출) 제외 시 노출 665 / 클릭 69 / **평균순위 5.50**.
- ⚠️ **how-to-lower-car-insurance는 8/24 CTR 작업 후에도 여전히 0클릭**(726→734노출, 순위 7.09 유지). title/meta 개선으로는 안 움직였다. 이 페이지는 포기하거나 다른 접근이 필요하다. **다음 세션에 또 손대지 말 것** — 두 번 시도해서 두 번 실패했다.
- 카테고리별 클릭: workers-comp 23 / business-insurance 9 / tools 7 / health 6 / home 6 / car 6 / blog 5 / life 4 / renters 3.
- GA4: 활성사용자 220(+11%). bing 45 + yahoo 17 + ddg 11 = **73세션** vs google 6 + search.google.com 9 = 15. **Bing 계열이 Google의 약 5배.**

## ✅ 이번 세션(9/8) 작업 — states 심화 2건
**선정 기준**: Bing 수익 상위 × 제도가 실제로 특이한 주(차별화 재료가 실재해야 한다). NJ(CRIB 요율산정)·MT(guaranteed market)가 잘 된 이유가 그것이었다.

1. **`states/workers-comp/texas.html`** (894→1,983단어, Bing 5노출 2클릭)
   미국 유일의 opt-out 주. 핵심 각도는 **"가입 안 함이 제도 밖이 아니다"** — non-subscriber도 4가지 의무가 있다: (a) DWC Form-005를 **매년 2/1~4/30 재신고**(1회 등록으로 오해해 수년째 미준수인 사업자가 다수라는 게 실무 최대 함정), (b) 사업장 게시(영어·스페인어), (c) 신규채용 시 서면통지, (d) 5인 이상은 DWC Form-007 매월 7일까지 재해보고.
   - 포기하는 것: 기여과실·위험인수·동료과실 **3대 항변권 상실** → 민사 직접 피소, 보상 상한 없음. 제소기간도 subscriber 1년 vs non-subscriber 2년.
   - occupational injury plan은 비용은 줄지만 **항변권을 되돌려주지 않는다**(중재합의는 별건)는 점을 명시 — 이 부분을 흐리게 쓰는 경쟁사가 많다.
   - **FAQ 4개 + FAQPage 스키마 신규**. 이 페이지엔 원래 FAQ가 아예 없었다.
2. **`states/workers-comp/north-dakota.html`** (1,083→2,297단어, Bing 11노출 1클릭)
   독점기금 주(WSI가 공급자이자 규제기관, 민영 인수 자체가 불법).
   - **최대 발견: 주기금 정책에는 employer's liability(Part Two)가 없다.** GL에 stop-gap 특약을 붙여야 하는데 WSI 계정 개설 과정에서 아무도 안 알려준다. 독점주 4곳 공통 이슈라 **OH·WA·WY 페이지에도 그대로 적용 가능**(다음 세션 후보).
   - 전 직원 근무 개시 '전' 가입(유예 없음), 임금 공제 불법·권리포기 각서 무효, 미가입 시 소급보험료 + 일시 $10,000 + 일 $100, 허위신고는 보험료 10배.
   - 독립계약자 면제는 WSI 검증신청이 필요하고 **유효기간 1년**(갱신 누락이 함정).
   - 주 경계 양방향 문제: 유입은 significant contacts 3요건(25% 기준), 유출은 WSI가 주 밖 인수 불가(연속 30일 한도) → **타주 원격근무자는 해당 주 정책이 별도로 필요**. 원격근무 확산으로 실무 빈발.
   - FAQ 4→9.

**효과**: 타 주 대비 본문 중복률 미수정 **44.9% → TX 20.4% / ND 19.8%**. 누적 5개 주(NJ·MT·CA·TX·ND) 차별화 완료.

**검증**: 459파일 / ld+json 파싱 실패 0, div·table 밸런스 0, 깨진 내부링크 0, sitemap 458 유효, 문서 순서(FAQ→Related Tools→xstate-links) 사이트 관행 일치 확인.

---

## 🎯 다음 작업 우선순위 (9/8)

**P0. 8/31·9/2에 이어 3주째 미확인 — GSC "수동 조치"·"보안 문제" 탭.** 알고리즘 강등이면 경고가 없지만 수동 조치면 재심사 경로가 생겨 대응이 완전히 달라진다. 내가 볼 수 없는 데이터다. **이것만 확인해주면 전략 판단이 크게 달라진다.**

**P1. states 심화 계속 — 주당 2~3개.** 이게 유일하게 회수가 확인된 작업이다.
- **다음 순번(Bing 클릭/노출 기준)**: maine(27/3), maryland(8/3), arizona(14/2), tennessee(13/2), indiana(7/1), wisconsin, georgia(1노출 3클릭 — 노출 대비 클릭 이상치, 확인 가치 있음).
- **효율 높은 묶음**: 독점기금 나머지 3주(**Ohio·Washington·Wyoming**). ND에서 쓴 stop-gap 논리가 그대로 적용되고, 각 주 고유 요소(OH의 BWC·그룹 요율, WA의 시간당 부과·근로자 분담, WY의 산업 한정)만 조사하면 된다. ND 작업이 템플릿 역할을 한다.
- 조사 없이 문장만 바꾸면 의미 없다는 원칙 유지.

**P2. business-insurance(중복 62%)·renters-insurance(59%) 카테고리.** workers-comp(45%)보다 나쁘다. business-insurance는 Bing 클릭 9개로 가치가 있다.

**P3. 제휴(Hiscox/CJ) 검토 — 4주째 이월.** Bing 클릭 69개, 그중 B2B(workers-comp 23 + business 9) 32개. 계속 미루고 있으니 다음엔 결정하든 목록에서 빼든 할 것.

**❌ 하지 말 것**:
- **how-to-lower-car-insurance CTR 재시도** — 두 번 실패했다.
- **states 페이지 noindex / 대량 삭제** — Bing 클릭 대부분이 거기서 나온다.
- **신규 페이지 대량 발행** — 3주간 12개 발행해서 회수 0이라는 실측이 있다. 심화가 우선.
- **Google 지표로 단기 성패 판정** — 판정 기준은 Bing.
- workers comp 클래스코드 격자, 신체부위별 보상액(법률 리스크), GL 계산기 신규, Ezoic, IndexNow 재론.

## 💰 수익화 (9/8)
Google 클릭 0 / Bing 클릭 69. Bing 단일 채널이고 3주 연속 성장 중이다(38→56→69). B2B 비중이 46%(32/69)로 제휴 단가가 가장 높은 트래픽이 절반 가까이 된다. **P3 제휴 검토가 계속 밀리는 게 지금 가장 아까운 지점**이다 — 트래픽은 있는데 수익화 장치가 없다.
